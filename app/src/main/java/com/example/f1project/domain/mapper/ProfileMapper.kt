...
object ProfileMapper {
    ...
    fun mapDriverProfile(
        ...
        // ZMIANA: sumujemy jako Double — zachowujemy precyzję punktów ułamkowych
        // Było: raceResults.sumOf { it.points.toInt() } — cicho ucinało 0.5 pkt
        // Jest:  raceResults.sumOf { it.points }        — pełna precyzja Double
        val stats = DomainSeasonStats(
            points  = raceResults.sumOf { it.points },
    ...
    fun mapConstructorProfile(
        ...
        // ZMIANA: sumujemy jako Double — pełna precyzja
        // Było: .toDoubleOrNull()?.toInt() ?: 0 — zbędna konwersja do Int
        val stats = DomainSeasonStats(
            points  = allResults.sumOf { it.points.toDoubleOrNull() ?: 0.0 },
            wins    = allResults.count { it.position == "1" },
            podiums = allResults.count { it.position.toIntOrNull()?.let { p -> p <= 3 } ?: false }
        )
